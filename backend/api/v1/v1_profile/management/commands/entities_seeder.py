...
def create_entity_data(self, administration: Administration, test: bool):
    ...
    for t in entity_types:
        ...
        name = (
            f"{entity.name} - {fake.company()}"
            if created
            else f"{t} - {fake.company()}"
        )
        if not test:
            self.stdout.write(name)
        EntityData.objects.create(
            name=name, administration=administration, entity=entity
        )
...
def seed_data(self, repeat: int = 3, test: bool = False):
    ...
    for adm in adms:
        create_entity_data(self, administration=adm, test=test)
    ...
    users = SystemUser.objects.filter(
        user_access__role=UserRoleTypes.user, user_form__form=entity_form
    ).order_by("?")[:repeat]
    for user in users:
        path = f"{user.user_access.administration.path}"
        path += f"{user.user_access.administration.id}"
        village = (
            Administration.objects.filter(path__startswith=path)
            .order_by("?")
            .first()
        )
        create_entity_data(self, administration=village, test=test)
    if len(users) == 0 and len(adms) == 0:
        ...
        for r in randoms:
            create_entity_data(self, administration=r, test=test)

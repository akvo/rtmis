...
class DataVisualisationTestCase(TestCase):
    def setUp(self):
        super().setUp()
        call_command("generate_views", "-f", "./source/config/category-example.json")
    ...
    def test_maps_data(self):
        ...
        data = self.client.get(
            "/api/v1/maps/{0}".format(form.id), follow=True, **header
        )
        self.assertEqual(data.status_code, 400)

        data = self.client.get(
            "/api/v1/maps/{0}?shape={1}&marker={2}".format(form.id, shape, marker),
            follow=True,
            **header,
        )
        self.assertEqual(data.status_code, 200)
        self.assertEqual(
            list(data.json()[0]), ["id", "loc", "name", "geo", "marker", "shape"]
        )
        for d in data.json():
        ...
        # Test Map Overview
        data = self.client.get(
            "/api/v1/maps/county/{0}".format(form.id), follow=True, **header
        )
        self.assertEqual(data.status_code, 400)
        data = self.client.get(
    ...
    def test_chart_data(self):
        ...
        question = Questions.objects.filter(
            form=form, type=QuestionTypes.option
        ).first()
        option = QuestionOptions.objects.filter(question_id=question.id).first()
        advance_filter = "{0}||{1}".format(option.id, option.name)

        data = self.client.get(
            "/api/v1/chart/data/{0}".format(form.id), follow=True, **header
        )
        self.assertEqual(data.status_code, 400)
        data = self.client.get(
        ...
        data = self.client.get(
            "/api/v1/chart/administration/{0}?question={1}&administration={2}".format(
                form.id, question.id, administration.id
            ),
            follow=True,
            **header,
        )
        ...
        self.assertEqual(list(data.json().get("data")[0]), ["group", "child"])
        self.assertEqual(
            list(data.json().get("data")[0]["child"][0]), ["name", "value"]
        )
        ...
        url = "/api/v1/chart/criteria/{0}?cache=example-criteria".format(form.id)
        payload = [
            {
                "name": "Test",
                "options": [
        ...
        data = self.client.post(
            url.format(form.id), payload, content_type="application/json"
        )
        self.assertEqual(data.status_code, 200)

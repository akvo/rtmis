...
describe("components/Header", () => {
  ...
  beforeEach(() => {
    axios.mockResolvedValue({ status: 200, data: fakeForm });
  });

  test("test if Header has a clickable logo", async () => {
    const { getByTestId } = render(<Header />, { wrapper: MemoryRouter });
    const logo = await getByTestId("logo");
    expect(logo).toHaveAttribute("href");
    fireEvent.click(logo, { button: 0 });
    let homePage;
    await waitFor(() => {
      homePage = render(<Home />, { wrapper: MemoryRouter });
    });
    expect(screen.getByText(/About RUSH/i)).toBeInTheDocument();
    expect(homePage.asFragment()).toMatchSnapshot();
  });

  test("test if dashboard is hoverable with drop down", async () => {
    const { getByTestId } = render(<Header />, { wrapper: MemoryRouter });
    const dashboard = await getByTestId("dashboard");
    fireEvent.mouseOver(dashboard);
  });

  test("test if report is hoverable with drop down", async () => {
    const { getByTestId } = render(<Header />, { wrapper: MemoryRouter });
    const report = getByTestId("report");
    fireEvent.mouseOver(report);
  });
});
